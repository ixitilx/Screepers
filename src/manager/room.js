...
{
    const current = _.countBy(structures, s => s.structureType)

    function iteratee(type)
    {
        const structuresNow = current[type] === undefined ? 0 : current[type]
        const structuresRcl = CONTROLLER_STRUCTURES[type][rcl]
        const structuresToDo = structuresRcl - structuresNow
        if(structuresToDo > 0)
            return {type: type, count: structuresToDo}
        {}
    }

    const todo = _(buildPriority).map(iteratee).filter(item => item && item.count).value()
...
function roomBuild(room)
{
    ...
    const spawns = _.filter(structures, structureType===STRUCTURE_SPAWN)
    if(_.size(spawns)===1)
    {
        ...
        {
            // find existing?
            const container = _(spawn.pos.lookAround(LOOK_STRUCTURES))
                                .map(rec => rec.structure)
            ...
            else
            {
                ...
                // _.filter(spawn.pos.lookAround(LOOK_TERRAIN)
                // // where?
                // let newPos = spawn.pos
                // while(spawn.pos.getRangeTo(newPos)===0 || newPos.)
                // {
                //     const getDelta = () => Math.floor(Math.random() * 3) - 1
                //     newPos = new RoomPosition(spawn.pos.x + getDelta(), spawn.pos.y + getDelta, spawn.pos.roomName)
                // }

                // const ret = room.createConstructionSite(newPos, STRUCTURE_CONTAINER)
            }
        }
    }
...
function controllerUpgrade(room, spawn)
{
    ...
    const terrainPos = _.map(terrain, 'pos')
